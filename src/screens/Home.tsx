...
export function Home() {
  ...
  // La alerta llega a los 2,5 s dentro de la misma tarjeta héroe: solo cambia el contenido (fundido),
  // la tarjeta no se desmonta ni se vuelve a montar, así el layout no salta.
  useEffect(() => {
    if (!state.alertNever && !state.alertSeen && !enPausa) later(() => { set({ alertSeen: true }); setAlerta(true); }, 2500);
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, []);

  const modo: 'pausa' | 'alerta' | 'estado' = enPausa ? 'pausa' : alerta ? 'alerta' : 'estado';
  const fade = { initial: { opacity: 0 }, animate: { opacity: 1 }, exit: { opacity: 0 }, transition: { duration: rm ? 0 : 0.35 } };
  ...
  return (
    <Screen tabs greeting titulo={`${saludo()}, ${PERSONA.nombre.toLowerCase()}.`}
      ...
      <motion.section className="hero mt-4" initial={{ opacity: 0, y: rm ? 0 : 10 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.45, ease: [0.2, 0.8, 0.2, 1] }} role={modo === 'alerta' ? 'status' : undefined}>
        <div className="hero__glow" />
        <AnimatePresence mode="wait" initial={false}>
          {modo === 'pausa' && state.evaluacion ? (
            <motion.div key="pausa" {...fade}>
              ...
              <div className="row" style={{ marginTop: 12, gap: 20 }}>
                ...
                <div className="row__grow"><h2 className="h1">Descansa.</h2><p className="sub mt-1">{rangoTexto(state.evaluacion.inicio, state.evaluacion.fin)}</p></div>
              </div>
              <div className="mt-5"><Button variant="primary" onClick={() => go('proceso')}>Ver mi proceso</Button></div>
            </motion.div>
          ) : modo === 'alerta' ? (
            <motion.div key="alerta" {...fade}>
              <button className="iconbtn" aria-label="No mostrar más" style={{ position: 'absolute', right: -4, top: -8, color: 'rgba(255,255,255,.7)' }} onClick={() => { set({ alertNever: true }); setAlerta(false); }}><Icon name="x" size={20} /></button>
            ...
            <motion.div key="estado" {...fade}>
              <p className="eyebrow">Tu ritmo esta semana</p>
              <div className="row" style={{ marginTop: 12, gap: 20 }}>
                <Ring estado="elevado" dias={PATRON.diasSinDesconectar} dark />
                <div className="row__grow"><h2 className="h1">Elevado.</h2><p className="sub mt-1">{PATRON.horasSemana} horas conectada y {PATRON.nochesTarde} noches trabajando tarde.</p></div>
              </div>
