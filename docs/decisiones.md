...
### D-24 · QA en dispositivo: instalado; el recorrido automatizado depende de que el teléfono esté desbloqueado
- Teléfono: Xiaomi 25100RA69G, Android 16 (API 36), HyperOS, 1080×2392 a 450 dpi. Es el objetivo de prueba principal del prompt.
- Primer intento de `adb install` cancelado por HyperOS (`INSTALL_FAILED_USER_RESTRICTED`); tras abrir Opciones de desarrollador en el teléfono, el segundo `adb install -r` terminó con `Success`. El APK también quedó en `/sdcard/Download/PAUSA-debug.apk`.
- HyperOS bloquea `input tap` e `input keyevent` desde adb (`SecurityException: INJECT_EVENTS`) salvo que se active "Depuración USB (ajustes de seguridad)" con cuenta Mi. Por eso el QA en dispositivo no usa eventos del sistema: `scripts/qa-dispositivo.mjs` conecta Playwright al WebView por DevTools (`adb forward … localabstract:webview_devtools_remote_<pid>`) y toma las capturas con `screencap`.
- La conexión funcionó (`https://localhost/`, `Capacitor.isNativePlatform() = true`, viewport 384×810 dp): Capacitor ya deja el WebView dentro de las barras del sistema, con `--safe-area-inset-*` en 0 px y las barras pintadas del color crema del fondo; en la única captura útil del primer intento (Inicio) nada queda tapado por la barra de estado ni por la de navegación.
- El recorrido completo no pudo terminar porque la pantalla del teléfono se apagó y quedó bloqueada (`mWakefulness=Dozing`, `mDreamingLockscreen=true`): con la pantalla apagada el WebView no pinta y `screencap` devuelve negro. Se dejó `svc power stayon usb` para que no vuelva a apagarse mientras esté conectado. Falta correr el script con el teléfono desbloqueado.
- **No verificado aún en dispositivo:** háptica, hoja de compartir, botón atrás físico (requiere el ajuste de seguridad de HyperOS o probarlo a mano con la lista de `docs/COMO-INSTALAR.md` §C) y fluidez a 60 fps.
